...
function wingen(winBackbone, configuration)
{
    ...
    self.qOffspringNonReferences = function(schemaProp, genProps, parentProps, overrideJSON)
    {
        return Q.fcall(function()
        {
            ...
            //send back the array of children
            return [children, parentIxs];
        })
    } 
    ...
    {   
        ...
        if(rBehavior == undefined)
        {
            ...
            for(var c=0; c < genProps.count; c++){
                children.push([]);
                parentIxs.push([]);
            }
        ...
        //send back promises or children 
        return {promises: promises, children: children, parentIxs: parentIxs};
    }
    ...
    self.qCreateChildReferences = function(childrenAndParentIxs, schemaProps, genProps, parentProps, overrideJSON)
    {
        var count = genProps.count; 
        ...
        var childrenToUpdate = childrenAndParentIxs.children;
        if(childrenToUpdate.length != count)
            throw new Error("By reference creation, there should be the appropriate number of children:" + count + " Not: " + childrenToUpdate.length);
        
        var tChildren = [];
        for(var c=0; c < childrenToUpdate.length; c++)
            tChildren.push(traverse(childrenToUpdate[c]))
        ...
        .then(function(refBehavior)
        {
            ...
            for(var rp in refPaths)
            {
                ...
                if(action.children)
                    pathToChildren[rp] = {offspring: action.children, parentIxs: action.parentIxs};
        ...
        .then(function(settled)
        {
            ...
            self.log("Behavior so far: ", pathToChildren);
            ...
            for(var i=0; i < settled.length; i++)
            {
                ...
                pathToChildren[path] = offReturn.offspringObject;
            }


            var parentIxs = childrenAndParentIxs.parentIxs;
            //now we have everything we need theoretically
            for(var rp in refPaths)
            {
                var childObjects = pathToChildren[rp];
                ...
                if(!childObjects || !childObjects.offspring || childObjects.offspring.length != count)
                {
                    throw new Error("Child objects not created properly: " + rp + 
                        " err: " + (childObjects && childObjects.offspring ? " wrong number of children " + childObjects.offspring.length : "-- no children found"));
                }
                ...
                for(var c=0; c < count; c++)
                {
                    tChildren[c].set(objPath, childObjects.offspring[c]);
    ...
    self.qCreateChildren = function(type, schemaProps, genProps, parentProps, overrideJSON)
    {
        var defer = Q.defer();
        ...
        .then(function(childrenAndIxs)
        {
            //pull children and parental info
            children = childrenAndIxs[0];

            //parentIxs is optional after non-reference objects
            parentIxs = childrenAndIxs[1];

            self.log('Nonref children: ', children);

            //must have children -- non optional
            if(!children || children.length != count || !parentIxs || parentIxs.length != count)
            {   
                throw new Error("CreateNonReference: Children and parent arrays returned must be non-empty equal to count: " + count);
            }
            var childrenAndParentIxs = {children: children, parentIxs: parentIxs};
            ...
            if(refCount)
                return self.qCreateChildReferences(childrenAndParentIxs, schemaProps, genProps, parentProps, overrideJSON);
            else
                return;
        })
        .then(function()
        {
            //defer returns NOW
            defer.resolve([children, parentIxs]);
        })
        .fail(function(err){
        ...
        qValidation(type, parents, self.validateParents)
            ...
            .then(function(results)
            {
                ...
                else
                {
                    ...
                    return self.qCreateChildren(type, schemaProps, genProps, parentProps, overrideJSON);
                }
